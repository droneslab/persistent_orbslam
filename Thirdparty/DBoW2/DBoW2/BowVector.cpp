...
bool BowVector::decreaseWeight(WordId id, WordValue v) //returning boolean to show if word still exist
{
  //std::cout << "bug in decreaseWeight " << id << " " << v << std::endl;
  BowVector::iterator vit = this->lower_bound(id);
  
  if(vit != this->end() && !(this->key_comp()(id, vit->first)))
  {
    //std::cout << "bug found id previous value " << vit->second << std::endl;
    vit->second -= v;
    //std::cout << "bug found id new value " << vit->second << std::endl;
...
double BowVector::normalize(LNorm norm_type)
{
  double norm = 0.0; 
