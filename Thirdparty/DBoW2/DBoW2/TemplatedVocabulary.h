...
public:
   ...
   */
  virtual void transform(const std::vector<TDescriptor>& features, std::vector<std::pair<unsigned int, double> >& keyToBow, std::vector<unsigned int>& keyToFeat,
...
template<class TDescriptor, class F> 
void TemplatedVocabulary<TDescriptor,F>::transform(
  const std::vector<TDescriptor>& features, std::vector<std::pair<unsigned int, double> >& keyToBow, std::vector<unsigned int>& keyToFeat,
  BowVector &v, FeatureVector &fv, int levelsup) const
{
  v.clear();
  fv.clear();
  ...
  if(m_weighting == TF || m_weighting == TF_IDF)
  {
    ...
    for(fit = features.begin(); fit < features.end(); ++fit, ++i_feature)
    {
      ...
      { 
        ...
        if (keyToBow.size())
        {
          keyToBow[i_feature].first = id;
          keyToBow[i_feature].second = w;

          keyToFeat[i_feature] = nid;
  ...
  if(must) {
    double our_norm = v.normalize(norm);
    if(our_norm > 0.0)
    {
      for (unsigned int i = 0 ; i < keyToBow.size(); i++)
      {
        keyToBow[i].second /= our_norm;
      }
    }
  }
}
