...
vector<KeyFrame*> KeyFrameDatabase::DetectLoopCandidates(KeyFrame* pKF, float minScore, bool vanilla_flag)
{
 ...
 //zaki_log this extra loop is for checking persistence of keyframes before BOW search, extra overhead but for now is OK!
   /* for(DBoW2::BowVector::const_iterator vit=pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit != vend; vit++)
    {
        ...
        for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++)
        {
            (*lit)->checkPersistence(pKF->mTimeStamp);
        }
        ...
        for(DBoW2::BowVector::const_iterator vit=pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit != vend; vit++)
        {
            if (!vanilla_flag)
            {
                ...
                for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++)
                {
                    ...
                    {
                        pKFi->mnLoopWords++;
                        if (pKFi->perm_words.find(vit->first) != pKFi->perm_words.end())
                            pKFi->mnPermWords++;
                    }
                }
            }
    ...
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        ...
        std::cout << pKFi->mnId << " " << pKFi->removed_so_far << " " << pKFi->removed_words << " " << pKFi->mnLoopWords << " " << minCommonWords << " " <<
        pKFi->mnPermWords << " " << pKFi->perm_words.size() << " ";
...
vector<KeyFrame*> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F, bool vanilla_flag)
{
    ...
    /*for(DBoW2::BowVector::const_iterator vit=F->mBowVec.begin(), vend=F->mBowVec.end(); vit != vend; vit++)
    {
        ...
        for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++)
        {
            (*lit)->checkPersistence(F->mTimeStamp);
        }
        ...
        for(DBoW2::BowVector::const_iterator vit=F->mBowVec.begin(), vend=F->mBowVec.end(); vit != vend; vit++)
        {
            if (!vanilla_flag)
            {
                ...
                //    {
                        ...
                        {
                            pKFi->getPersistentRelocWords();
                            pKFi->mnRelocWords=0;
                            pKFi->mnRelocPermWords=0;
                            pKFi->mnRelocQuery=F->mnId;
                            lKFsSharingWords.push_back(pKFi);
                        }
                        pKFi->mnRelocWords++;
                        if (pKFi->reloc_perm_words.find(vit->first) != pKFi->reloc_perm_words.end())
                            pKFi->mnRelocPermWords++;
                //    }
                }
            }
            else
    ...
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        ...
        if((*lit)->mnRelocPermWords>maxCommonPerm)
            maxCommonPerm=(*lit)->mnRelocPermWords;
    }

    int minCommonWords = maxCommonWords*0.8f;
    int minCommonPerm = maxCommonPerm*0.8f;
    ...
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        ...
        std::cout << pKFi->mnId << " " << pKFi->removed_so_far << " " << pKFi->removed_words << " " << pKFi->mnRelocWords << " " << minCommonWords << " " <<
        pKFi->mnRelocPermWords << " " << maxCommonPerm << " " << pKFi->perm_words.size() << " ";
        if(pKFi->mnRelocPermWords>minCommonPerm) //pKFi->mnRelocWords>minCommonWords && )
        {
            //zaki_change
            if (!vanilla_flag)
                pKFi->draw(F->mnId);
            ...
            if (!vanilla_flag)
            {
                ...
                si = mpVoc->score(F->mBowVec,pKFi->mBowVec_reloc_perm);
            }
        ...
        std::cout << mpVoc->score(F->mBowVec,pKFi->mBowVec) << " " << mpVoc->score(F->mBowVec,pKFi->mBowVec_reloc_perm) << " " << pKFi->mRelocScore;
