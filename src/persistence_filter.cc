  ...
  if(logLY_)  // If the running sum logLY has been previously initialized ...
    {
      ...
      {
        pre_logLY_ = boost::optional<double> (*logLY_);
      }
...
bool PersistenceFilter::update_reverse(double current_time)
{
  
  if (pre_tN_ < current_time)
  {
    if(pre_logLY_)  // If the running sum logLY has been previously initialized ...
      {
        // Update its value according to the paper
        *logLY_ = *pre_logLY_;
      }
