...
KeyFrame* Map::_ReadKeyFrame(ifstream &f, ORBVocabulary &voc, std::vector<MapPoint*> amp, ORBextractor* orb_ext) {
  ...
  KeyFrame* kf = new KeyFrame(fr, this, NULL, cv::Mat()); //zaki_change
...
MapPoint* Map::_ReadMapPoint(ifstream &f) {
  ...
  MapPoint* mp = new MapPoint(wp, mnFirstKFid, mnFirstFrame, this, 0); //zaki_change
  mp->mnId = id;
  return mp;
}
...
bool Map::Save(const string &filename) {
  ...
  bool details = (filename.compare("temp.map") != 0);
//  cerr << "  writing " << mspMapPoints.size() << " mappoints" << endl;
  std::cout << "total_map_points " << mspMapPoints.size() << endl;
  ...
  for(auto mp: mspMapPoints)
  {
    if (details)
    {
      std::cout << "overhead_map_point " << mp->mnId << " " << mp->overhead_time << std::endl << std::flush;
      std::cout << std::fixed << "map_point reobservation " << mp->mnId << " " << mp->mnFirstFrame << " " << mp->video_num << " " << mp->intra_reobservation << " " << mp->intra_missed_detection << " " << mp->inter_reobservation << " " << mp->inter_missed_detection << std::endl << std::flush;
    }
    _WriteMapPoint(f, mp);
  }
...
//  cerr << "  writing " << mspKeyFrames.size() << " keyframes" << endl;
  std::cout << "total_keyframes " << mspKeyFrames.size() << endl;
  ...
  for(auto kf: mspKeyFrames) {
    if (details)
      std::cout << "overhead_keyframe " << kf->mnId << " " << kf->overhead_time << std::endl;
...
//  cerr << "Map: saved " << st.st_size << " bytes" << endl;
  std::cout << "map_size " << st.st_size << endl;
