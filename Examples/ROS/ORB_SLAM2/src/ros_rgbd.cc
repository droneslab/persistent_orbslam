...
int main(int argc, char **argv)
{
    ...
    message_filters::Subscriber<sensor_msgs::Image> rgb_sub(nh, "/camera/rgb/image_color", 1); //"/camera/rgb/image_color", 1);
    message_filters::Subscriber<sensor_msgs::Image> depth_sub(nh, "/camera/depth/image", 1); // /camera/depth_registered/image_raw", 1);
...
/*    for (unsigned int i = 0; i < folder_names.size(); i++)
    {
        ...
        std::string file_name = "/home/zakieh/PhD/data/semantic_mapping/NetworksLab/" + folder_names[i] + "/rgb-depth/times.txt";
        ...
        while ( getline (myfile,line) )
        {
            cv::Mat rgb, depth;
            count_line++;
            file_name = "/home/zakieh/PhD/data/semantic_mapping/NetworksLab/" + folder_names[i] + "/rgb-depth/rgb" +line+ ".yml";
            cv::FileStorage fs;
            fs.open(file_name, cv::FileStorage::READ);
            fs["matName"] >> rgb;
            fs.release();

            file_name = "/home/zakieh/PhD/data/semantic_mapping/NetworksLab/" + folder_names[i] + "/rgb-depth/depth" +line+ ".yml";
            fs.open(file_name, cv::FileStorage::READ);
            fs["matName"] >> depth;
            fs.release();
...
/*        std::cout << "writing images" << std::endl;
        ...
        mpSLAM->TrackRGBD(cv_ptrRGB->image,cv_ptrD->image,lastPoseStamp_,false);
//    }
}
